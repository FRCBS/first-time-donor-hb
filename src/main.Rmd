...
basics = donation0 %>% # simple
  filter(!is.na(Hb)) %>%
  group_by(year,Sex) %>%
  summarise(n=n(),mean0=mean(Hb,na.rm=TRUE),sd0=sd(Hb,na.rm=TRUE),deferred.perc=sum(deferred)/n(),limit=min(limit),.groups='drop')
...
# data0=simple[simple$year==basics$year[i],]
for (i in 1:nrow(basics)) {
  rv = rectifyDistribution(simple[simple$year==basics$year[i],],basics$Sex[i],if(basics$Sex[i]=='Male') 135 else 125,FALSE)
...
# plot.cols=c('dperc0')
plotColBySex = function(basics,col,ylab) {
  par(mar=c(2,4,0,4))
  plot.cols=c(col)
  plot(basics$year,basics[[plot.cols[1]]],type='n',ylab=ylab)
  for (i in 1:nrow(sex.col)) {
    data1 = basics[basics$Sex==sex.col$sex[i],]
    for (col in plot.cols) {
      lines(data1$year,data1[[col]],col=sex.col$col[i],lwd=2)
      frml=paste0(col,'~year')
      m=lm(formula(frml),data=data1)
      sm=summary(m)
      print(sm)
      sm$coeff[2,4]
      abline(a=sm$coeff[1,1],b=sm$coeff[2,1],col=sex.col$col[i],lty='dashed',lwd=2)
      text(x=min(data1$year),y=min(data1[[col]])+0.01,labels=paste0('b=',round(100*sm$coeff[2,1],5),'%,p=',round(sm$coeff[2,4],3)),adj=c(0))
    }
  }
}
