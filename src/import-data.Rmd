...
importCountry = function(country,file) {
  file=file.path(param$wd,'results',file)
...
```{r test-ggplot}
library(ggplot2)
ggplot(basics.nl, aes(x=year, y=mean, color=Sex)) +
  geom_line() +
  geom_smooth(method='lm', formula=y~x) 

ggplot(basics.nl, aes(x=year, y=n, color=Sex)) + 
    geom_line() +
  geom_smooth(method='lm', formula=y~x)

ggplot(basics.nl, aes(x=year, y=deferred.prop, color=Sex)) + 
    geom_line() +
  geom_smooth(method='lm', formula=y~x)

ggplot(basics.nl, aes(x=year, y=age, color=Sex)) + 
    geom_line() 
  # geom_smooth(method='lm', formula=y~x)
