...
if (param$country == 'NL') {
  ...
  param$donorkeys = c('releaseID', 'BloodGroup')
  param$hb.minimum = 1
  param$hb.maximum = 20
  ...
  param$hb.decimals = 0
  param$donorkeys = c('releaseID', 'DateOfBirth', 'Sex', 'BloodGroup')
...
```{r load-and-procoess-data}

if (is.null(param$donation.type.keys)) 
  param$donation.type.keys = unique(donationdata$donation$BloodDonationTypeKey)


#MP: take only columns that are necessary for safety, otherwise join can make things funny in NL
simple = donationdata$donation[, c("releaseID", "BloodDonationTypeKey", "DonationDate", "DonationPlaceType", "DonationPlaceCode", "Hb", "DonationTimeStart")] %>%
...
anonymizeDistribution = function(freq) {
  ...
  aggregated = freq[c(wh.sub,wh.super),] %>%
    ...
    summarise(Hb=min(sgn)*hb.inf,n2=sum(n),nas=sum(nas),deferred=sum(deferred),
              mean.age=sum(n*mean.age)/sum(n),sd.age=NA,limit=min(limit),.groups='drop') %>%
    dplyr::select(-sgn) %>%
    rename(n=n2)
...
write.xlsx(list(parameters=data.frame(name=names(param),value=paste(param,sep=',')),
           ...
           file=file.path(param$wd,'results','exported-statistics.xlsx'))
...
newLMapproach = function(basics) {
  ...
  plot(deferred.prop~mean,data=data,ylim=c(0,0.15),col='red')
  ...
  plot(data$year,sm1.female$residuals,col='red',type='l',ylab='residuals',lwd=3)
