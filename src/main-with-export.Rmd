...
simple = donationdata$donation %>%
  ...
  left_join(simple,donationdata$donor[,c('releaseID','DateOfBirth','Sex','BloodGroup')],by='releaseID')
...
for (ds in data.sets) {
  ...
  c.freq = if(is.null(c.freq)) freq else rbind(c.freq,freq)
}
...
for (ds in data.sets) {
  freq = get(ds) %>%
    ...
    summarise(n=n(),.groups='drop')
  c.age.freq = if(is.null(c.freq)) freq else rbind(c.age.freq,freq)
}
...
for (ds in data.sets) {
  monthly = get(ds) %>%
    ...
    summarise(n=n(),mean=mean(Hb,na.rm=TRUE),sd=sd(Hb,na.rm=TRUE),mean.age=mean(age,na.rm=TRUE),sd.age=sd(age,na.rm=TRUE),nas=sum(1*is.na(Hb)),.groups='drop') 
  c.monthly = if(is.null(c.monthly)) monthly else rbind(c.monthly,monthly)
}
```

```{r export-data}
write.xlsx(list(annual.hb=c.freq,annual.age=c.monthly,montly.statistics=c.monthly),file=file.path(param$wd,'exported-statistics.xlsx'))
```
...
```{r}
# Monthly averages from repeat donors
seasonal = donation.r %>%
  group_by(month) %>%
...
# repeat donations augmented with the seasonal data
...
basics = donation0 %>% # simple
  filter(!is.na(Hb)) %>%
  group_by(year,Sex) %>%
  summarise(n=n(),mean0=mean(Hb,na.rm=TRUE),sd0=sd(Hb,na.rm=TRUE),deferred.perc=sum(deferred)/n(),
