...
param$cutoff.male = 135 # 8.4 # will need to convert these to correct units
param$cutoff.female = 125 # 7.8
...
simple = donationdata$donation %>%
  ...
  # filter(!is.na(Hb)) %>% #filter NA's # TODO must decide whether to keep or leave
...
date0 = simple %>%
  filter(param$country != 'NL' || BloodDonationTypeKey == "New") %>%
  group_by(releaseID) %>%
...
```{r export-data}
write.xlsx(list(annual.hb=hb.freq,annual.age=age.freq,montly.statistics=monthly),
