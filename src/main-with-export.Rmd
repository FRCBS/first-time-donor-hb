...
```{r}
getMoments = function(data,data.set='donation0',value.col='Hb',group.col='year',n.filter=100) {
  # data = age.freq
  ...
  wh = min(which(colnames(data)==value.col))
  colnames(data)[wh]='value'
  wh.group = min(which(colnames(data)==group.col))
  colnames(data)[wh.group]='group'

  new.basics.mean = data %>%
    # filter(country=='FI') %>%
    filter(data.set=='donation0',!is.na(value)) %>% # These are the summary rows for NA; leave them out at this point
    group_by(group,Sex) %>%
    summarise(n2=sum(n-nas),mean0=sum(value*(n-nas))/n2,.groups='drop')
  ...
  new.basics = data %>%
    ...
    group_by(group,Sex) %>%
    summarise(mean=min(mean0),n0=min(n),n=min(n2),var=sum(prop*dev^2),sd=sqrt(var),
...
new.basics = getMoments(hb.freq,'donation0','Hb')
