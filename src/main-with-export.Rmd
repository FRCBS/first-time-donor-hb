...
simple = donationdata$donation %>%
  ...
  left_join(donationdata$donor[, param$donorkeys],by='releaseID')
...
# simple$hour = as.integer(format(simple$DonationDateDTTM,'%H'))
#NL take the first substr from "%H-M%"
#simple$hour <- ifelse(is.na(simple$DonationTimeStart), NA_integer_, as.integer(substr(simple$DonationTimeStart, 1, 2)))
