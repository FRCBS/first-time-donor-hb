...
param$country = 'FI' # Not used currently; country could also be assigned when reading in the various data
```
...
anonymizeDistribution = function(freq) {
  # freq = hb.freq
  wh.sub = which(freq$n < 5 & freq$Hb < freq$limit)
  wh.super = which(freq$n < 5 & freq$Hb > freq$limit)
  
  freq$Hb[wh.sub] = -1e6
  freq$Hb[wh.super] = 1e6
  
  aggregated = freq[c(wh.sub,wh.super),] %>%
    mutate(sgn=sign(Hb)) %>%
    group_by(data.set,year,Sex,sgn) %>%
    summarise(Hb=min(sgn)*1e6,n2=sum(n),nas=sum(nas),deferred=sum(deferred),
...
```{r export-data}
write.xlsx(list(annual.hb=hb.freq,annual.age=age.freq,montly.statistics=monthly),file=file.path(param$wd,'results','exported-statistics.xlsx'))
```
...
rectifyDistribution = function (data0,sex=NULL,cutoff=NULL,plot=TRUE,freq=NULL) {
  if (is.null(cutoff)) {
    ...
    cutoff = if (sex=='Female') 125 else 135
  }
...
rectifyDistribution(NULL,'Female',125,freq=hb.freq[hb.freq$Sex=='Female',])
