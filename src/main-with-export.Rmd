...
```{r getStats-function}
getStats = function(freq,data.set='donation0',value.col='Hb',group.col='year',n.filter=100,age.freq=NULL,age.breaks=c(-Inf,24,40,Inf),age.labels=c('young','mid','old')) {
  # data = age.freq
...
new.basics = getStats(hb.freq,'donation0','Hb',age.freq=age.freq)
...
plotColBySex(new.basics,'old','proportion')
```


```{r statistics-by-age}
agedist = getStats(age.freq,data.set='all',value.col='mean.hb',group.col='age',n.filter=100)
